...
from third_party.processors.utils_top import (
...
def main():
    ...
    ## Other parameters
    parser.add_argument("--intent_labels", default="", type=str,
                        help="Path to a file containing all labels. If not specified, NER/POS labels are used.")
    parser.add_argument("--slot_labels", default="", type=str,
                        help="Path to a file containing all labels. If not specified, NER/POS labels are used.")
    ...
    if args.do_predict_dev and args.local_rank in [-1, 0]:
        ...
        with open(output_test_results_file, "w") as result_writer:
            for lang in args.predict_langs.split(','):
                # if not os.path.exists(os.path.join(args.data_dir, lang, 'dev.{}'.format(suffix))):
                if not os.path.exists(os.path.join(args.data_dir, "dev-{}.jsonl".format(lang))):
                    logger.info("Language {} does not exist".format(lang))
                    continue
                result, predictions = evaluate(
                    args, model, tokenizer, (intent_labels, slot_labels), pad_token_label_id, mode="dev",
