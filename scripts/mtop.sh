...
DATA_DIR=$DATA_DIR/mtop_udpipe_processed

export CUDA_VISIBLE_DEVICES=$GPU
LANGS="en,es,fr,de,hi"

if [[ "$USE_SYNTAX" == 'true' ]]; then
    SAVE_DIR="${OUT_DIR}/mtop/syntax-seed${SEED}"
else
    SAVE_DIR="${OUT_DIR}/mtop/seed${SEED}"
fi
mkdir -p $SAVE_DIR;

export PYTHONPATH=$HOME_DIR;
python $HOME_DIR/third_party/top.py \
    --seed $SEED \
    --data_dir $DATA_DIR \
    --intent_labels $DATA_DIR/intent_label.txt \
    --slot_labels $DATA_DIR/slot_label.txt \
    --model_type bert \
